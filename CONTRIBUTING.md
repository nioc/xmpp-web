...
You can add provided git `commit-msg` [hook](docs/git-hooks/commit-msg) (which execute a basic message checking) with the following command: `pnpm run configure-git-hook`.
...
2. Build the project `pnpm install` and wait for the downloads
3. Start the vite server `pnpm run dev`
...
4. When you are satisfied with your work, run the linter `pnpm run lint`
